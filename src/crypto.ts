...
import type { AgentIdentity, SessionKeys } from './types.js';
...
function bufferToBase64(v: Buffer | string): string {
  return Buffer.isBuffer(v) ? v.toString('base64') : Buffer.from(v).toString('base64');
}
...
export function generateIdentity(): AgentIdentity {
  ...
  const publicKeyRaw = readRawFromSpki(
  ...
  return {
    algorithm: 'ed25519',
    publicKeyDerBase64: bufferToBase64(Buffer.from(publicKeyDer)),
    privateKeyDerBase64: bufferToBase64(Buffer.from(privateKeyDer)),
...
export function signTranscriptHashBase64(
  ...
  const signature = sign(
    null,
    Buffer.from(transcriptHashBase64, 'base64'),
    privateKey
  );
